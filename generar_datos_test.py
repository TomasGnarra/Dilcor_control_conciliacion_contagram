...
df_ventas = df_ventas.iloc[1:-1].copy()  # Skip header row and totals row
...
# Top clients go to specific banks for realism
banco_assignment = {}
for i, c in enumerate(clientes):
    if i < 5:
        # Top 5 clientes: Mercado Pago (alto flujo)
        banco_assignment[c["cliente"]] = "mercadopago"
    elif i < 15:
        # Siguientes 10: mix Galicia/Santander
        banco_assignment[c["cliente"]] = random.choice(["galicia", "santander"])
    else:
        # Resto: distribución ponderada
...
def gen_alias_banco(cliente, banco):
    """Genera descripción bancaria realista (como aparece en extracto)."""
...
def split_into_payments(monto_total, cliente_nombre):
    """Divide un monto mensual en pagos individuales realistas."""
    payments = []
    ...
    while remaining > 10000:
        ...
        if len(payments) >= 8:  # Max 8 payments per client
            if remaining > 0:
...
# --- COBRANZAS (entradas de dinero de clientes) ---
for item in tabla_parametrica:
    ...
    for pay in payments:
        # 85% match exacto, 10% monto ligeramente distinto, 5% sin referencia clara
        match_type = random.choices(["exacto", "parcial", "sin_ref"], weights=[0.85, 0.10, 0.05])[0]

        descripcion = item["alias_banco"]
        monto = pay["monto"]

        if match_type == "parcial":
            # Pequeña diferencia (retención bancaria, comisión, etc.)
            monto = round(monto * random.uniform(0.97, 0.995), 2)
            descripcion = descripcion + " -RET"
        elif match_type == "sin_ref":
            # Descripción genérica sin referencia clara
            if banco == "mercadopago":
...
# --- PAGOS (salidas de dinero a proveedores) ---
for item in tabla_parametrica:
    if item["tipo"] != "Proveedor":
        continue
    # Pagos salen del Galicia y Santander principalmente
...
# --- Agregar comisiones bancarias y movimientos extras ---
...
for tx in sorted(extractos["mercadopago"], key=lambda x: x["fecha"]):
    monto = tx["monto"]
    if tx["tipo"] == "CREDITO":
        comision = round(monto * 0.045, 2)  # 4.5% comisión MP
...
print(f"Tabla Paramétrica:    {len(df_param)} registros")
...
print(f"\nTotal créditos bancarios: ${total_creditos:,.2f}")
print(f"Total débitos bancarios:  ${total_debitos:,.2f}")
